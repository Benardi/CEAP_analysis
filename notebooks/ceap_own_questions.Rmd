...
We will examine the top 10 most expensive suppliers (in terms of median cost).

```{r}
data %>%
  mutate(fornecedor = toupper(fornecedor)) %>%
  group_by(fornecedor, sgPartido) %>%              # calc expense of each
  summarize(total_expense = sum(valorLíquido)) %>% # supplier for each party
  ungroup() %>%
  group_by(fornecedor) %>%                           # calc median expense of
  mutate(median_expense = median(total_expense)) %>% # each supplier (all parties)
...
* Now that we  have the median cost of each supplier we can examine the overall behavior of the top 10 most expensive suppliers.
